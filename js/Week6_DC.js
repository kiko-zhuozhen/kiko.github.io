function lastItem(fruits, outputId) {
    ...
    if (!outputDiv.innerHTML) {
        outputDiv.innerHTML = `Fruit sorted: ${fruits.join(', ')} <br> Last Alphabetical Item: ${lastAlphabetical}`;
    }
}
...
function getAndSort() {
    ...
    displayArea.innerHTML= `<h4> You entered: </h4> <ul> ${items.map(item => `<li>${item}</li>`).join('')} </ul>`;
    displayArea.innerHTML += `<h4> Sorted By: </h4> <ul>${sortedItems.map(item => `<li>${item}</li>`).join('')} </ul>`;
}
